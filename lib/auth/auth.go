...
func (a *Server) CreateAuthenticateChallenge(ctx context.Context, req *proto.CreateAuthenticateChallengeRequest) (*proto.MFAAuthenticateChallenge, error) {
	...
	challenges, err := a.mfaAuthChallenge(ctx, username, scope, req.AllowReuse)
	if err != nil {
		log.Error(trace.DebugReport(err))
...
func (a *Server) mfaAuthChallenge(ctx context.Context, user string, scope webauthnpb.ChallengeScope, allowReuse bool) (*proto.MFAAuthenticateChallenge, error) {
	...
	if len(groupedDevs.Webauthn) > 0 {
		...
		assertion, err := webLogin.Begin(ctx, user, scope, allowReuse)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		challenge.WebauthnChallenge = wantypes.CredentialAssertionToProto(assertion)
...
func (a *Server) ValidateMFAAuthResponse(ctx context.Context, resp *proto.MFAAuthenticateResponse, user string, passwordless bool) (*types.MFADevice, string, error) {
	...
	case *proto.MFAAuthenticateResponse_Webauthn:
		...
		if passwordless {
			...
			dev, err = webLogin.Finish(ctx, user, wantypes.CredentialAssertionResponseFromProto(res.Webauthn), resp.Scope, resp.AllowReuse)
		}
		if err != nil {
