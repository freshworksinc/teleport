...
Edit  the `module` block in `main.tf` as follows:
...
1. Assign `agent_count` to `2` for high availability. As you scale your Teleport
   ...
   You can consider adding your agents to an auto-scaling group as well, though
   this will require copying and modifying the `agent-pool-terraform` module.
...
1. Modify the elements of the `agent_roles` list.
   ...
   The values of `agent_roles` also determine which Teleport services each agent
   deployed by the module will run.
...
1. If you are deploying your instance in a demo environment and do not have a
   NAT gateway, NAT instance, or other method for connecting your instances to
   the Teleport Proxy Service, modify the `module` block to associate a
   public IP address with each agent instance: 
...
|PROXY_SERVICE_ADDRESS|The host and port of the Teleport Proxy Service, e.g., `example.teleport.sh`|
...
## Step 3/4. Edit the agent configuration

Each virtual machine launched by the module runs a script on startup This script
installs Teleport on the host, then writes a configuration file to the default
location, `/etc/teleport.yaml`. The configuration file enables each Teleport
service we associated with our token.

If the `agent_roles` input variable includes the `Node` role, the configuration
also adds the `role: agent-pool` label to the Teleport SSH Service on each
instance. This makes it easier to access hosts in the agent pool later.

Finally, the script restarts Teleport on the host to apply the new
configuration.

In the default configuration, only the Teleport SSH Service is enabled.
...
Each cloud-specific module declares resources to deploy a virtual machine
