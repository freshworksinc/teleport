...
- Cloud infrastructure that enables virtual machine instances to connect to the
  ...
  For demo purposes, you can also configure the VM instances you deploy to have
  public IP addresses, and we will show you how to do so in this guide.
...
1. Assign a value to `agent_roles`. 
...
Each virtual machine launched by the module runs a script on startup, which we
configured above using the `user_data` field within the
`aws_instance.teleport_agent` resource
(`examples/agent-pool-terraform/teleport/userdata`):

```yaml
(!examples/agent-pool-terraform/agent-config.yaml!)
```
...
In the default configuration, only the Teleport SSH Service is enabled. Edit
`examples/agent-pool-terraform/agent-config.yaml` to enable each Teleport
service you plan to run on your agent nodes by making the following change for
each service you want to enable:
...
Ensure that the value of the `agent_roles` field in the `module` block
corresponds to all of the Teleport services you have enabled in the
configuration.
