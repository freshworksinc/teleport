...
Each virtual machine launched by the module runs a script on startup that
installs Teleport on the host, then writes a configuration file to the default
location, `/etc/teleport.yaml`. 

The configuration file enables each Teleport service you associate with the join
token using the `agent_roles` input variable. For example, if the value of
`agent_roles` is `["Node", "Db"]`, the configuration file enables the Teleport
SSH Service and Teleport Database Service.

If the value of the `agent_roles` input variable includes the `Node` role, the
configuration also adds the `role: agent-pool` label to the Teleport SSH Service
