...
Configure the `teleport_provision_token` resource to include a role that
corresponds to each service you plan to run. For example, if you enable the
Teleport Kubernetes Service, the token must have the `Kube` role.
